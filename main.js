...
const phoneNumber = await question(color('Masukin No Awal 628 Jangan Masukin Nama VinXD Ganteng🗿🗿 Ga Bisa Bang🗿:\n', 'red'));
...
Skyzo.sendMessage("6283153419618@s.whatsapp.net", {text: "*BOT OPEN BANG VinXD 😜*"})
...
if (ff.isGroup == false) {
if (ff.status == "offer") {
let sendcall = await Skyzo.sendMessage(ff.from, {text: `@${ff.from.split("@")[0]} Maaf Kamu Akan Saya Block Karna Ownerbot Menyalakan Fitur *Anticall*\nJika Tidak Sengaja Segera Hubungi Owner Untuk Membuka Blokiran Ini`, contextInfo: {mentionedJid: [ff.from], externalAdReply: {thumbnailUrl: "https://k.top4top.io/p_3312s70dh0.png", title: "｢ CALL DETECTED ｣", previewType: "PHOTO"}}}, {quoted: null})
...
Skyzo.sendMessage(anu.id, {text: check ? `@${anu.author.split("@")[0]} Telah Mengeluarkan sikontol yang bernama  @${num.split("@")[0]} Dari Grup Ini` : `@${num.split("@")[0]}  sikontol Telah Keluar Dari Grup Ini kalau mau edit sc gni ke VinXD aja 6283153419618`, 
...
Skyzo.sendMessage(anu.id, {text: `@${anu.author.split("@")[0]} Telah Menjadikan sang raja @${num.split("@")[0]} Sebagai Admin Grup Ini`, 
...
Skyzo.sendMessage(anu.id, {text: `@${anu.author.split("@")[0]} Telah Memberhentikan sampah @${num.split("@")[0]} Sebagai Admin Grup Ini`, 
